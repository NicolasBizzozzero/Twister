function init(){	
        ...
        env.key=0;	
	test();
	var s=makeMainPanel(env.id,env.login);
...
function makeMainPanel(fromId,fromLogin){
	alert("entree dans makeMainPanel");
...
function CompleteMessages(){
	if(!env.noConnection){}
		...
		var tab = getFromLocalDb(env.fromId,env.minId,env.maxId,var tab=[],10);
		CompleteMessageReponse(JSON.stringify(tab));
	}
}
